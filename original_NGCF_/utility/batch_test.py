...
'''
import original_NGCF_.utility.metrics as metrics
from original_NGCF_.utility.parser import parse_args
from original_NGCF_.utility.load_data import *
...
def test(sess, model, users_to_test, save_recommendation_files = False, drop_flag=False, batch_test_flag=False):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        for i, user in enumerate(user_batch):
            ...
            scores = rate_batch[i].copy()

            # mask training items (adjust attribute name to your codebase)
            train_items = data_generator.train_items[user]  # or data_generator.train_set[user], etc.
            scores[list(train_items)] = -np.inf

            topk = 100
            top_items = np.argpartition(-scores, topk)[:topk]
            top_items = top_items[np.argsort(-scores[top_items])]  # sorted by score desc

            row = {
                "user_id": int(user),
                "user_ground_truths": list(test_items[i]),
                "user_top_k_prediction": top_items.tolist(),
                # optional but useful:
                # "user_top_k_scores": scores[top_items].tolist()
            }
            all_rows.append(row)
