...
class Trainer(AbstractTrainer):
    ...
    def fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            # train
            print(f"Epoch number:  {epoch_idx + 1}")
    ...
    def evaluate(
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            ...
            user_ids = interaction["user_id"].detach().cpu()
            user_ids = user_ids.tolist()
            # OPTIONAL:
            # If training items are NOT already masked inside _full_sort_batch_eval,
            # then you must mask them manually here.
            #
            # Example only if you have access to user history items:
            #
            # for row, u in enumerate(user_ids.tolist()):
            #     seen_items = ...   # get training/history items for user u
            #     scores[row, seen_items] = -float("inf")

            # top-k recommendation indices
            topk_scores, topk_items = torch.topk(scores, k=50, dim=-1)

            topk_items = topk_items.detach().cpu()
            topk_scores = topk_scores.detach().cpu()

            # ground truth from positive_u / positive_i
            gt_dict = {}
            pu = positive_u.detach().cpu().tolist()
            pi = positive_i.detach().cpu().tolist()
            
            for row_idx, item_id in zip(pu, pi):
                
                real_user_id = user_ids[row_idx]
                gt_dict.setdefault(real_user_id, []).append(item_id)

            # save one row per user
            for row_idx, u in enumerate(user_ids):
                rec_items = topk_items[row_idx].tolist()
                rec_scores = topk_scores[row_idx].tolist()
                gt_items = gt_dict.get(u, [])
    ...
    @torch.no_grad()
    def evaluate_test(
        self, eval_data, train_data, load_best_model=True, model_file=None, show_progress=False
    ):
        recommendation_rows = []
        r"""Evaluate the model based on the eval data.
        ...
        if not self.config["single_spec"]:
            result = self._map_reduce(result, num_sample)
        self.wandblogger.log_eval_metrics(result, head="eval")
        return result, recommendation_rows
