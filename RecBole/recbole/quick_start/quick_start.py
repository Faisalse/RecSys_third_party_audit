...
def test(train):
    ...
    all_have_interactions = all(count > 0 for count in user_counts.values())
    print(all_have_interactions)

    dataset = train.dataset
    uid_field = train.uid_field

    # get all user ids from interaction data
    uids = dataset.inter_feat[uid_field].numpy().tolist()

    # count interactions per user
    user_counts = Counter(uids)

    print("Number of users:", len(user_counts))
    print("Minimum interactions per user:", min(user_counts.values()))
    print("Maximum interactions per user:", max(user_counts.values()))
...
def run_recbole(
    ...
    out_path = Path("results/"+config["model"]+"/"+config["dataset"]+"/")
    out_path.mkdir(parents=True, exist_ok=True)
    recommendation.to_csv(out_path / "recommendation_files.csv", index=False, sep = "\t")
